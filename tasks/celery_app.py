...
def init_redis():
    ...
    try:
        # 从Celery配置中提取Redis连接信息
        # Celery结果后端格式: redis://host:port/db
        result_backend = set_config.CELERY_RESULT_BACKEND
        if result_backend.startswith('redis://'):
            ...
            logger.error(f"Unsupported result backend: {result_backend}")
