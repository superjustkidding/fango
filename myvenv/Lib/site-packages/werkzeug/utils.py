...
def append_slash_redirect(environ: WSGIEnvironment, code: int = 308) -> Response:
    ...
    If the current URL is ``/users/42``, the redirect URL will be
    ``42/``. When joined to the current URL during response
    processing or by the browser, this will produce ``/users/42/``.
...
def send_file(
    ...
    intend. Use :func:`send_from_directory` to safely serve users-provided paths.
