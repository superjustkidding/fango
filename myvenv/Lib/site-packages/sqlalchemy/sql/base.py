...
class _DialectArgDict(MutableMapping[str, Any]):
    ...
    Maintains a separate collection of users-specified arguments
...
class DialectKWArgs:
    ...
    def argument_for(
        ...
        argument names is typically for end-users code that is also using
...
class CompileState:
    ...
    on converting raw users intent into more organized structures rather than
...
class Executable(roles.StatementRole):
    ...
    def execution_options(self, **kw: Any) -> Self:
        ...
        **execution options never affect the compiled SQL of a query, only
        ...
        dictionaries may be used for users-defined parameters that interact with
...
class ColumnCollection(Generic[_COLKEY, _COL_co]):
    ...
    def add(
        ...
        .. note::
            ...
            This method is **not normally used by users-facing code**, as the
