...
class _AbstractLoad(traversals.GenerativeOnTraversal, LoaderOption):
    ...
    def contains_eager(
        ...
        the desired rows, i.e.::
            ...
            sess.query(Order).join(Order.users).options(contains_eager(Order.users))

        The above query would join from the ``Order`` entity to its related
        ``User`` entity, and the returned ``Order`` objects would have the
        ``Order.users`` attribute pre-populated.
    ...
    def joinedload(
        ...
        :param innerjoin: if ``True``, indicates that the joined eager load
         should use an inner join instead of the default of left outer join::
            ...
            select(Order).options(joinedload(Order.users, innerjoin=True))
...
class _AttributeStrategyLoad(_LoadElement):
    ...
    def _init_path(
        ...
        # here we assume we have users-passed InstrumentedAttribute
