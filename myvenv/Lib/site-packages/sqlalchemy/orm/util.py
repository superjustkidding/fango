...
class _TraceAdaptRole(enum.Enum):
    ...
    polymorphic" parameters, with whole users-defined subqueries. The
    ...
    users-explicit aliasing schemes using the ``aliased()`` and
...
def _is_mapped_annotation(
    ...
    try:
        ...
        # involved with end-users annotations that have nothing to do with us.
