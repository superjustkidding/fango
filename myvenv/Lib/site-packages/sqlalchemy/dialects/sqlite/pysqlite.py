...
.. sourcecode:: text
    ...
    driver://users:pass@host/database
...
operator ``REGEXP`` that calls a users-defined function that must be provided.
...
method that allows us to create our own users-defined functions (UDFs) in Python and use them directly in SQLite queries.
