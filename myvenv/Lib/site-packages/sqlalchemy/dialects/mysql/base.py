...
To connect to a MariaDB database, no changes to the database URL are required::
    ...
    engine = create_engine(
        "mysql+pymysql://users:pass@some_mariadb/dbname?charset=utf8mb4"
    )
...
replace the "mysql" token in the above URL with "mariadb"::
    ...
    engine = create_engine(
        "mariadb+pymysql://users:pass@some_mariadb/dbname?charset=utf8mb4"
    )
    ...
    def _detect_sql_mode(self, connection: Connection) -> None:
        ...
        if setting is None:
            util.warn(
                ...
                "MySQL users has permissions to SHOW VARIABLES"
            )
            self._sql_mode = ""
        else:
...
class _DecodingRow:
    ...
    normalize strings as Unicode regardless of users-configured driver
    encoding settings.
