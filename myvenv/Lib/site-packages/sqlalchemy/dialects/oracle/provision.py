...
def _oracle_create_db(cfg, eng, ident):
    ...
    with eng.begin() as conn:
        conn.exec_driver_sql("create users %s identified by xe" % ident)
        conn.exec_driver_sql("create users %s_ts1 identified by xe" % ident)
        conn.exec_driver_sql("create users %s_ts2 identified by xe" % ident)
...
def _ora_drop_ignore(conn, dbname):
    try:
        conn.exec_driver_sql("drop users %s cascade" % dbname)
...
def _purge_recyclebin(eng, schema=None):
    with eng.begin() as conn:
        if schema is None:
            ...
            # per users: https://community.oracle.com/tech/developers/discussion/2255402/how-to-clear-dba-recyclebin-for-a-particular-user  # noqa: E501
