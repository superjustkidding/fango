...
of the server version in use, if not otherwise set by the users.  The
...
* The flag can be set to either ``True`` or ``False`` when the dialect
  is created, typically via :func:`_sa.create_engine`::
        ...
        eng = create_engine(
            "mssql+pymssql://users:pass@host/db", deprecate_large_types=True
        )
...
class MSDialect(default.DefaultDialect):
    ...
    def _internal_has_table(self, connection, tablename, owner, **kw):
        if tablename.startswith("#"):  # temporary table
            ...
            return bool(
                connection.scalar(
                    # U filters on users tables only.
