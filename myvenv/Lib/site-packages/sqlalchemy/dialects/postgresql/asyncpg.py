...
.. dialect:: postgresql+asyncpg
    :name: asyncpg
    :dbapi: asyncpg
    :connectstring: postgresql+asyncpg://users:password@host:port/dbname[?key=value&key=value...]
...
:func:`_asyncio.create_async_engine` engine creation function::
    ...
    engine = create_async_engine(
        "postgresql+asyncpg://users:pass@hostname/dbname"
    )
...
present, otherwise an exception is raised::
    ...
    engine = create_async_engine(
        "postgresql+asyncpg://users:password@/dbname?host=HostA:5432&host=HostB:5432&host=HostC:5432"
    )
...
argument)::
    ...
    engine = create_async_engine(
        "postgresql+asyncpg://users:pass@hostname/dbname?prepared_statement_cache_size=500"
    )

To disable the prepared statement cache, use a value of zero::

    engine = create_async_engine(
        "postgresql+asyncpg://users:pass@hostname/dbname?prepared_statement_cache_size=0"
    )
...
a prepared statement is prepared::
    ...
    engine = create_async_engine(
        "postgresql+asyncpg://users:pass@somepgbouncer/dbname",
        poolclass=NullPool,
        connect_args={
...
client using this setting passed to :func:`_asyncio.create_async_engine`::
    ...
    engine = create_async_engine(
        "postgresql+asyncpg://users:password@localhost/tmp",
...
class PGDialect_asyncpg(PGDialect):
    ...
    def create_connect_args(self, url):
        opts = url.translate_connect_args(username="users")
