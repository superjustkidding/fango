...
.. dialect:: postgresql+psycopg2
    :name: psycopg2
    :dbapi: psycopg2
    :connectstring: postgresql+psycopg2://users:password@host:port/dbname[?key=value&key=value...]
...
which specifies Unix-domain communication rather than TCP/IP communication::
    ...
    create_engine("postgresql+psycopg2://users:password@/dbname")
...
using ``host`` as an additional keyword argument::
    ...
    create_engine(
        "postgresql+psycopg2://users:password@/dbname?host=/var/lib/postgresql"
    )
...
.. warning::  The format accepted here allows for a hostname in the main URL
   ...
   format, the initial host is silently ignored**.  That is, this URL::
        ...
        engine = create_engine(
            "postgresql+psycopg2://users:password@myhost1/dbname?host=myhost2"
        )
...
and ``HostC::PortC``::
    ...
    create_engine(
        "postgresql+psycopg2://users:password@/dbname?host=HostA:PortA&host=HostB&host=HostC:PortC"
    )
...
in the comma separated list::
    ...
    create_engine(
        "postgresql+psycopg2://users:password@/dbname?host=HostA,HostB,HostC&port=PortA,,PortC"
    )
...
For example, to indicate two hosts using the ``primary`` strategy::
    ...
    create_engine(
        "postgresql+psycopg2://users:password@/dbname?host=HostA:PortA&host=HostB&host=HostC:PortC&target_session_attrs=primary"
    )
...
* For PostgreSQL 9.1 and above, the ``client_encoding`` parameter may be
  ...
  ``libpq`` PostgreSQL client library::
    ...
    engine = create_engine(
        "postgresql+psycopg2://users:pass@host/dbname?client_encoding=utf8"
    )
  ...
  ``libpq``::
    ...
    engine = create_engine(
        "postgresql+psycopg2://users:pass@host/dbname",
