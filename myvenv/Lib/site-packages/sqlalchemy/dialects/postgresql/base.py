...
.. admonition:: Section Best Practices Summarized
    ...
    match remote schemas, or ensure the ``"$users"`` token is **removed** from
...
Where above, the ``"$users"`` variable will inject the current username as the
...
.. sidebar:: Best Practices for PostgreSQL Schema reflection
    ...
    users preferences. But in fact, there's no need to understand any of it if
...
class PGDialect(default.DefaultDialect):
    ...
    def _columns_query(self, schema, has_filter_names, scope, kind):
        ...
        query = (
            ...
            .select_from(pg_catalog.pg_class)
            # NOTE: postgresql support table with no users column, meaning
