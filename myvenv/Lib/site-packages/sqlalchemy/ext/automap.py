...
asking it to reflect the schema and produce mappings::
    ...
    # engine, suppose it has two tables 'users' and 'address' set up
...
explicit table declaration::
    ...
    metadata.reflect(engine, only=["users", "address"])
...
``users`` and ``address``, we can define one or both of the classes to be used::
    ...
    # pre-declare User for the 'users' table
    class User(Base):
        __tablename__ = "users"
...
8. Production of the :func:`_orm.relationship` and optionally the
   ...
   function, which can be supplied by the end-users in order to augment
...
bidirectional relationship pair ``Address.users`` and
...
def generate_relationship(
    ...
    **kw: Any,
    ...
    :param return_fn: the function that is used by default to create the
     ...
     so it is critical
     that users-defined implementations correctly differentiate between the two
