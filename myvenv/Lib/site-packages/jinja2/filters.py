...
def do_xmlattr(
    ...
    **Values** that are neither ``none`` nor ``undefined`` are automatically
    escaped, safely allowing untrusted users input.

    User input should not be used as **keys** to this filter. If any key
    contains a space, ``/`` solidus, ``>`` greater-than sign, or ``=`` equals
    sign, this fails with a ``ValueError``. Regardless of this, users input
...
def do_sort(
    ...
    .. sourcecode:: jinja
        ...
        {% for users in users|sort(attribute="name")
    ...
    .. sourcecode:: jinja
        ...
        {% for users in users|sort(attribute="age,name") %}
            ...
        {% endfor %}
...
def sync_do_selectattr(
    ...
    .. code-block:: python
        ...
        (users for users in users if users.is_active)
        (users for users in users if test_none(users.email))
...
def sync_do_rejectattr(
    ...
    .. code-block:: python
        ...
        (users for users in users if not users.is_active)
        (users for users in users if not test_none(users.email))
