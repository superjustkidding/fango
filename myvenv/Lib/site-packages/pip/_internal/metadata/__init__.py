...
def _should_use_importlib_metadata() -> bool:
    ...
    * On Python 3.11, 3.12 and 3.13, Python distributors can patch
      ...
      ``pkg_resources`` (unless the users set the aforementioned environment
      variable to *True*).
...
def _emit_pkg_resources_deprecation_if_needed() -> None:
    ...
    if hasattr(importlib.metadata, "_PIP_USE_IMPORTLIB_METADATA"):
        ...
        # warn, since it is not a users decision.
        return

    # The users has decided to use pkg_resources, so we warn.
    deprecated(
